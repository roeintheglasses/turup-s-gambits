...
export function OpponentArea({
  ...
  return (
    ...
    >
      ...
      <div className={`flex ${getCardBacksLayout()} items-center justify-center`}>
        {Array.from({ length: visibleCards }).map((_, i) => (
          <motion.div
            ...
            className="w-8 h-11 sm:w-9 sm:h-12 md:w-10 md:h-14 rounded-md bg-gradient-to-br from-[#8B4513] via-[#A0522D] to-[#654321] border-2 border-[#5D3A1A] shadow-md"
            style={{
              marginLeft: position !== "left" && position !== "right" && i > 0 ? "-20px" : "0",
              marginTop: (position === "left" || position === "right") && i > 0 ? "-28px" : "0",
              zIndex: i,
            }}
          >
            {/* Card back design */}
        ...
        {hiddenCards > 0 && (
          <div className="ml-1 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded">
            +{hiddenCards}
          </div>
        )}
      </div>
      ...
      {/* Player info badge */}
      <div
        className={`relative flex items-center gap-2 px-3 py-1.5 rounded-lg border-2 transition-all duration-300 ${teamColor} ${teamBg} ${
      ...
      >
        ...
        <div className="flex flex-col">
          <span className="text-xs sm:text-sm font-medieval text-white font-semibold leading-tight">
            {name}
          </span>
